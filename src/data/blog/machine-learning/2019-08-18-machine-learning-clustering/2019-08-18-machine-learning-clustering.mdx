...
$$\color{orange}{L(r, s) = min(D(x_{ri}, x_{sj})) \tag{}}$$
...
$$\color{orange}{L(r, s) = max(D(x_{ri}, x_{sj})) \tag{}}$$
...
$$\color{orange}{L(r, s) = \frac{1}{n_r n_s} \sum_{i=1}^{n_r}\sum_{j=1}^{n_s} D(x_{ri}, x_{sj}) \tag{}}$$

### K-Means Clustering

K-Means clustering intends to partition n objects into k clusters in which each object belongs to the cluster with the nearest mean. This method produces exactly k different clusters of greatest possible distinction. The best number of clusters k leading to the greatest separation (distance) is not known as a priori and must be computed from the data. The objective of K-Means clustering is to minimize total intra-cluster variance, or, the squared error function:

$$\color{orange}{\text{Objective function} \rightarrow J = \sum_{j=1}^k \sum_{i=1}^n \| x_i^{(j)} - c_j \|^2 \tag{}}$$
...
$$\color{orange}{s(i, k) = - \| x_i - x_k \|^2 \tag{}}$$
...
$$\color{orange}{r(i, k) \leftarrow s(i, k) - max_{k's.t.k' \neq k} \{a(i, k') + s(i, k')\} \tag{}}$$
...
$$\color{orange}{a(i, k) \leftarrow min\{0, r(k, k) + \sum_{i'.s.t.i' \notin (i, k)} max(0, r(i', k)\} \tag{}}$$
...
$$\color{orange}{a(k, k) \leftarrow \sum_{i' \neq k} max(0, r(i', k)) \tag{}}$$

The final examplars are chosen by the maximum value of $A + B$

$$\color{orange}{\text{exemplar}(i, k) = max\{ a(i', k) + b(i', k) \} \tag{}}$$
