...
$$\color{orange}{\frac{p}{1-p} = exp(b_0 + b_1 x) \tag{}}$$
...
$$\color{orange}{ln \left(\frac{p}{1-p}\right) = b_0 + b_1 x \tag{}}$$

As mentioned before, logistic regression can handle any number of numerical and/or categorical variables.

$$\color{orange}{p = \frac{1}{1 + e^{-(b_0 + b_1 x_1 + b_2 x_2 + \dots + b_p x_p)}} \tag{}}$$
...
$$\color{orange}{\beta^1 = \beta^0 + [X^TWX]^{-1} X^T(y - \mu) \tag{}}$$
...
| Pseudo R2  | Equation  | Description |
|---|---|---|
...
|  McFadden's | $R^2 = 1 - \frac{LL_{\text{full model}}}{LL_{intercept}}$  |The ratio of the log-likelihoods suggests the level of improvement over the intercept model offered by the full model.|
|  Count | $R^2 = \frac{\text{No. Corrects}}{ \text{Total Count}}$  |The number of records correctly predicted, given a cutoff point of .5 divided by the total count of cases. This is equal to the accuracy of a classification model.|
...
$$\color{orange}{W_j = \frac{b_j}{SE_{b_j}} \tag{}}$$
...
$$
J(\theta) = \frac{1}{m} \sum_{i=1}^m Cost(h_{\theta}(x^{(i)}), y^{(i)}) \\[0.7em]
...
$$\color{orange}{J(\theta) = -\frac{1}{m} \sum_{i=1}^m [y^{(i)} log(h_{\theta}(x^{(i)})) \ + \ (1-y^{(i)})  log(1 - h_{\theta}(x^{(i)}))] \tag{}}$$
...
$$
\color{orange}{Z = \beta_1 x_1 + \beta_2 x_2 + \dots + \beta_d x_d \\[0.7em]
...
$$
\color{orange}{\beta = C^{-1}(\mu_1 - \mu_2) \\[0.8em]
...
$$\color{orange}{P(c|x) = \frac{P(x|c) \ P(c)}{P(x)} \tag{}}$$
...
$$\color{orange}{log_2 P(c|x) - log_2 P(c) \tag{}}$$
...
$$\color{orange}{Info(D) = - \sum_{i=1}^m p(i) log_2 p(i) \tag{}}$$

where, $p(i)$ is the probability that an arbitrary tuple in $D$ belongs to class $c(i)$.

$$
\color{orange}{Info_A(D) = \sum_{j=1}^v \frac{|D_j|}{|D|} \times Info(D) \\[1em]
...
$$\color{orange}{SplitInfo_A(D) = - \sum_{j=1}^v \frac{|D_j|}{|D|} \times log_2 \left(\frac{|D_j|}{|D|}\right) \tag{}}$$
...
$$\color{orange}{\text{Gain Ratio(A)} = \frac{Gain(A)}{SplitInfo_A(D)} \tag{}}$$
...
$$\color{orange}{Gini(D) = 1 - \sum_{i=1}^m p(i)^2 \tag{}}$$
...
$$\color{orange}{Gini_A(D) = \frac{|D_1|}{|D|} Gini(D_1) + \frac{|D_2|}{|D|} Gini(D_2) \tag{}}$$
...
$$\color{orange}{\Delta Gini(A) = Gini(D) - Gini_A(D) \tag{}}$$
...
$$\color{orange}{\frac{votes_i}{k} \tag{}}$$
...
$$
\color{orange}{min \frac{1}{2} \| w \| ^2 \\[0.7em]
